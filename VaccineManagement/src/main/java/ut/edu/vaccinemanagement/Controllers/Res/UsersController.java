...
public class UsersController {
    ...
    public ResponseEntity<Appointment> bookAppointment(@RequestBody AppointmentRequest request) {
        ...
        Appointment appointment = appointmentService.bookAppointment(
                userId,               // userId
                request.getChildId(), // childId
                request.getVaccineId() // vaccineId
        );
    ...
    public ResponseEntity<Child> addChild(@RequestBody Child child) {
        try {
            ...
            // Tạo liên kết User ↔ Child
            UserChild userChild = new UserChild();
            userChild.setUser(userService.getUserById(userId)); // Lấy thông tin User từ userId
            userChild.setChild(savedChild);
            userChildService.saveUserChild(userChild); // Lưu liên kết
    ...
    public ResponseEntity<List<Vaccine>> getAllVaccine() {
        List<Vaccine> vaccines = vaccineService.getAllVaccine(); // Gọi Service để lấy dữ liệu
        if (vaccines.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NO_CONTENT).body(vaccines); // Nếu không có vaccine, trả về 204
        }
        return ResponseEntity.ok(vaccines); // Trả về danh sách vaccine với mã trạng thái 200
    }
